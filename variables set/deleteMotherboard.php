   ...
   endforeach;
	...
	foreach ($prodTrans as $cool):
	  ...
	  if (!in_array($cool['transaction_id'], $TransArray)) {
		...
		case '/deleteMotherboard.php':                   // URL (without file name) to a default screen
			require 'dontOwn.php';
			break; 
	...
	$sqlDelProd = "DELETE FROM project.motherboard WHERE Product_ID = '$prod_ID'";
	...
	$sqlDelTrans = "DELETE FROM project.transaction_product WHERE transaction_id = '$transId'";
	if ($db->query($sqlDelTrans) === TRUE) {
	//echo "deleted transProd success";
	} else {
	  echo "Error: " . $sqlDelTrans . "<br>" . $db->error;
	  }
	//echo "<br>";
	
	$sqlDelTransProf = "DELETE FROM project.transaction_profile WHERE transaction_id = '$transId'";
	if ($db->query($sqlDelTransProf) === TRUE) {
	//echo "deleted transProf success";
	} else {
	  echo "Error: " . $sqlDelTransProf . "<br>" . $db->error;
	  }
	//echo "<br>";
