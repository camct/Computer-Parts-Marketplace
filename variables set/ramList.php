...
<?php foreach ($whole as $cool): ?>
  <tr>
	...
	<td><?php echo $cool['Price']; ?></td>	 
     <td><?php echo $cool['Mem_size']; ?></td>
	 
	<td>
	<form name="ramDelForm" action="deleteRAM.php" method="post">   
	...
	<td>
	<form name="ramTransForm" action="updateRAMForm.php" method="post">   
	<div class="row mb-1 mx-1">
    <input type="hidden" name="prodID" value=<?php echo $cool['Product_ID']; ?> />
	...
	<td>
	<form name="ramBuyForm" action="buyRAM.php" method="post">   
	<div class="row mb-1 mx-1">
    <input type="hidden" name="prodID" value=<?php echo $cool['Product_ID']; ?> />
...
<form method="GET">
        ...
        if (isset($_GET['min_price']) && isset($_GET['max_price'])) {
          ...
          $query = "SELECT * FROM project.ram WHERE ram.price > $min_price AND ram.price < $max_price";

        }
        else {
          $query = "SELECT * FROM project.ram";
        }
        ...
        if(mysqli_num_rows($whole)>0) {
            foreach($whole as $cool):
              ?>
                ...
                <h4><?php echo "Memory Size: " . $cool['Mem_size']; ?></h4>
                <br>
                <hr>
                <?php
            endforeach;
...
<a href="ramForm.php">Click here to add a new item of this type</a>
<br>
