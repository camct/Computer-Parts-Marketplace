...
<?php foreach ($whole as $cool): ?>
  <tr>
	 ...
	 <td><?php echo $cool['Voltage']; ?></td>	 
	 <td><?php echo $cool['Current_rating']; ?></td>
<td>
	<form name="psuDelForm" action="deletePSU.php" method="post">   
	...
	<td>
	<form name="psuTransForm" action="updatePSUForm.php" method="post">   
	<div class="row mb-1 mx-1">
    <input type="hidden" name="prodID" value=<?php echo $cool['Product_ID']; ?> />
	...
	<td>
	<form name="psuBuyForm" action="buyPSU.php" method="post">   
	<div class="row mb-1 mx-1">
    <input type="hidden" name="prodID" value=<?php echo $cool['Product_ID']; ?> />
...
<form method="GET">
        ...
        if (isset($_GET['min_price']) && isset($_GET['max_price'])) {
          ...
          $query = "SELECT * FROM project.psu WHERE psu.price > $min_price AND psu.price < $max_price";

        }
        else {
          $query = "SELECT * FROM project.psu";
        }
        ...
        if(mysqli_num_rows($whole)>0) {
            foreach($whole as $cool):
              ?>
                ...
                <h4><?php echo "Voltage: " . $cool['Voltage']; ?></h4>
				<h4><?php echo "Current rating: " . $cool['Current_rating']; ?></h4>
                <br>
                <hr>
                <?php
            endforeach;
...
<a href="psuForm.php">Click here to add a new item of this type</a>
<br>
